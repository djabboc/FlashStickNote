...
- 进入列表时必须将键盘焦点放到选中的 `ListBoxItem`；Up/Down 在列表内部移动并滚动，不能触发菜单或搜索框的方向导航。
- `Ctrl+B` 再次按下返回进入列表前的标题/正文焦点；正文使用每条笔记独立的 AvalonEdit 文档，因此返回已浏览的新笔记时保留该笔记的光标状态。
- 新增 `togglePin=Ctrl+P`，与右键菜单共用置顶命令。
- 切换置顶前同步保存当前有内容的未落盘笔记，配置立即写回；重新创建 ViewModel 必须恢复置顶。

**验收**：WPF harness 模拟 Down，确认列表选择变化且焦点仍在列表；再次调用 Ctrl+B 后正文得到焦点；配置重载后置顶状态仍为 true。
## 验证与提交
