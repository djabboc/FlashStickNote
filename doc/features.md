...
| focusNoteList | Ctrl+B | 在左侧列表与上次编辑位置间切换；进入列表时定位当前项 | 窗口内 |
| cyclePinnedNotes | F2 | 在置顶笔记间循环，目标滚入视野后恢复原焦点 | 窗口内 |
| togglePin | Ctrl+P | 为当前笔记增加/删除置顶标签 | 窗口内 |
...
- Ctrl+B 进入列表时焦点落在当前笔记项；上下键只浏览列表并保持列表焦点，再按 Ctrl+B 回到上次正文/标题编辑位置。
...
- `notesDir` 支持相对路径、绝对路径、`~/`、`%USERPROFILE%`；切换到自定义目录时可一次性搬入程序目录旧 `notes/` 中的文件，以 `.flashsticknote` 标记阻止重复导入；默认目录不创建该标记
