...
- 标题栏可隐藏（`hideTitleBar`）：无边框窗口，菜单栏右侧空白、列表底部空白和其他非交互背景均可拖动，边缘可调整大小；自定义拖动只在无标题栏时启用，关闭/最小化依赖快捷键与托盘
...
- 文件菜单：新建笔记、隐藏到托盘、退出；不再在列表顶部放置新建/删除按钮
- 笔记项右键菜单可删除对应笔记，仍遵从删除确认和回收站规则
